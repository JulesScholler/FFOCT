function handles=AnalogicSignalOCTFluo(handles) % Ne fonctionne que si (s.IsNotifyWhenScansQueuedBelowAuto=true); The only constraint is that the signal should be at least long of fs points
% This function generates an analogic trigger for the fluorescence camera
% and for the piezo and OCT camera in order to synchronize them.
...
handles.fluoCam.Ncam=floor(handles.fluoCam.Fcam);
for i=1:handles.fluoCam.Ncam
